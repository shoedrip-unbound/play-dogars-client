...
class BattleLog {
	...
	add(args: Args, kwArgs?: KWArgs, preempt?: boolean) {
		...
		let curRoom = app.rooms[this.scene?.battle!.roomid!] as BattleRoom;
		...
		case '@:':
			...
			switch (obj.cmd) {
				...
				case 'imgns':
					...
					break;
				case 'yt': {
					const hidden = `<details><summary>YouTube Video</summary><youtube src="https://www.youtube.com/embed/${obj.youtubeId}"/></details>`
			...
			case 'chatmsg-raw': case 'raw': case 'html':
				...
				if (args[1].includes('invite') &&
					...
					curRoom.battle.playbackState &&
					Dex.prefs("automodoff")) {
					curRoom.send('/modjoin off', 4 | 1);
				}
				break;
